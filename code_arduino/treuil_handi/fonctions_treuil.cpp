...
void treuil::init(char adr_encp, char adr_eeprom_nb_tour_pot, char adr_enct, char adr_eeprom_nb_tour_treuil, char moteur,char moteur_pwm, char moteur_left, char moteur_right)
{
  c_potar.set_compteur(adr_encp, adr_eeprom_nb_tour_pot);        // récupère la position du potar en eeprom
  c_treuil.set_compteur(adr_enct, adr_eeprom_nb_tour_treuil);    // récupère la position du treuil en eeprom
...
void treuil::marche()
{
  ...
  case 1:                                                      // mode position, fréquence de rafraichissement 10ms (100hz)
    ...
    //--------------gestion du jeux mecannique -------
    
    if (last_dir > 0 & pos_atteint == 0)
    {
      if (treuil_pos >= last_pos)                                // consigne depasse
      {
        pos_atteint = 1;
      }
    }
    else if (last_dir < 0 & pos_atteint == 0)
    {
      if (treuil_pos <= last_pos)                                // consigne depasse
      {
        pos_atteint = 1;
    ...
    if ((treuil_pos > last_pos + jeu_mini) & pos_atteint)       // jeu depasse
    {
      pos_atteint = 0;                                          // reactivation
      last_dir = -1;                                             //detection du sens
    }
    if ((treuil_pos < last_pos - jeu_mini) & pos_atteint)       // jeu depasse
    {
      pos_atteint = 0;                                          // reactivation
